...
class TriangleView: UIView {
    ...
    /// sin
    private let sinValue = sin(CGFloat.pi / 8)
    
    /// cos
    private let cosValue = cos(CGFloat.pi / 8)
    
    ///三角形
    private var outterTriangleLayer: CAShapeLayer = {
        let l = CAShapeLayer()
        l.strokeColor = contentTextWhiteColor().withAlphaComponent(1.0).cgColor
        l.fillColor = contentTextWhiteColor().cgColor
        l.opacity = 1
        l.lineWidth = 2
        return l
    }()
...
//    }()
    ...
    override func layoutSubviews() {
        ...
        outterTriangleLayer.path = drawTriangle(cornerRadius: R, centerToTop: 0.5, centerToLeft: 1, centerToRight: 1)
