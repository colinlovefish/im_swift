...
let kCircleHeightBackground: CGFloat = 0
...
open class SCLAlertView: UIViewController {
    ...
    public struct SCLAppearance {
        ...
        public init(kDefaultShadowOpacity: CGFloat = 0.7, kCircleTopPosition: CGFloat = 0.0, kCircleBackgroundTopPosition: CGFloat = 0.0, kCircleHeight: CGFloat = 0.0, kCircleIconHeight: CGFloat = 0.0, kTitleHeight:CGFloat = 25.0,  kWindowWidth: CGFloat = (UIScreen.main.bounds.size.width - 60 ), kWindowHeight: CGFloat = 178.0, kTextHeight: CGFloat = 90.0, kTextFieldHeight: CGFloat = 30.0, kTextViewdHeight: CGFloat = 80.0, kButtonHeight: CGFloat = 40.0, kTitleFont: UIFont = UIFont.systemFont(ofSize: 20), kTitleMinimumScaleFactor: CGFloat = 1.0, kTextFont: UIFont = UIFont.systemFont(ofSize: 16), kButtonFont: UIFont = UIFont.boldSystemFont(ofSize: 18), showCloseButton: Bool = true, showCircularIcon: Bool = false, shouldAutoDismiss: Bool = true, contentViewCornerRadius: CGFloat = 5.0, fieldCornerRadius: CGFloat = 3.0, buttonCornerRadius: CGFloat = 3.0, hideWhenBackgroundViewIsTapped: Bool = false, circleBackgroundColor: UIColor = UIColorFromRGB(0x191e31), contentViewColor: UIColor = UIColorFromRGB(0x191e31), contentViewBorderColor: UIColor = UIColorFromRGB(0x191e31), titleColor: UIColor = UIColorFromRGB(0xFFFFFF), subTitleColor: UIColor = UIColor.init(hex: "#FFFFFF", alpha: 0.6)!, margin: Margin = Margin(), dynamicAnimatorActive: Bool = false, disableTapGesture: Bool = false, buttonsLayout: SCLAlertButtonLayout = .vertical, activityIndicatorStyle: UIActivityIndicatorView.Style = .white, textViewAlignment: NSTextAlignment = .left) {
			...
			self.circleBackgroundColor = darkBlueColor()
            self.contentViewColor = darkBlueColor()
            self.contentViewBorderColor = darkBlueColor()
            self.titleColor = contentTextWhiteColor()
            self.subTitleColor = textWhiteColor()
...
//        view.backgroundColor =
        ...
        bottomLine.backgroundColor = lineWhiteColor()
    ...
    override open func viewWillLayoutSubviews() {
        ...
        if let title = labelTitle.text {
          titleActualHeight = title.heightWithConstrainedWidth(width: subViewsWidth, font: labelTitle.font) + 20
        ...
        windowHeight += viewText.text.isEmpty ? 0 : appearance.margin.textViewBottom + 20 // only viewText.text is not empty should have margin.
...
//        let titleOffset : CGFloat = appearance.showCircularIcon ? 0.0 : -12.0
        ...
        bottomLine.frame = CGRect.init(x: 0, y: y, width: contentView.frame.width, height: 1)
        contentView.addSubview(bottomLine)
        
        switch appearance.buttonsLayout {
        case .vertical:
            for btn in buttons {
                btn.frame = CGRect(x:buttonX, y: y + 15, width:subViewsWidth, height:appearance.kButtonHeight)
    ...
    @discardableResult
    fileprivate func addButton(_ title:String, backgroundColor:UIColor? = darkBlueColor(), textColor:UIColor? = TEXT_BLUE, showTimeout:SCLButton.ShowTimeoutConfiguration? = nil)->SCLButton {
        // Update view height
...
//        btn.backgroundColor = btn.customBackgroundColor ?? viewColor
    ...
    @discardableResult
    open func showInfo(_ title: String, subTitle: String? = nil, closeButtonTitle:String?=nil, timeout:SCLTimeoutConfiguration?=nil, colorStyle: UInt=SCLAlertViewStyle.info.defaultColorInt, colorTextButton: UInt=0x0A84FF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
...
//        circleBG.isHidden = !appearance.showCircularIcon
        ...
        for btn in buttons {
//             btn.customBackgroundColor = UIColorFromRGB(0x191e31)
//            if let customBackgroundColor = UIColorFromRGB(0x191e31) {
