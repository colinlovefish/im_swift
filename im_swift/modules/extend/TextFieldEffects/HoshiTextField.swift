...
@IBDesignable open class HoshiTextField: TextFieldEffects {
    ...
    */
    @IBInspectable dynamic open var placeholderFontScale: CGFloat = 1.1{
        didSet {
            updatePlaceholder()
        }
    }
    ...
    func errorInfomation(error : String) {
        if  error.count > 0 {
            errorLabel.isHidden = false
            errorLabel.text = error
            errorLabel.textColor = darkRedColor()
            activeBorderLayer.backgroundColor = darkRedColor().cgColor
        } else {
    ...
    private func rectForBorder(_ thickness: CGFloat, isFilled: Bool) -> CGRect {
        if isFilled {
            return CGRect(origin: CGPoint(x: 0, y: frame.height-thickness - 15), size: CGSize(width: frame.width, height: thickness))
        } else {
            return CGRect(origin: CGPoint(x: 0, y: frame.height-thickness - 15), size: CGSize(width: 0, height: thickness))
        }
    }
    ...
    private func layoutPlaceholderInTextRect() {        
        ...
        errorLabel.frame = CGRect(x: originX, y: textRect.height/2  + placeholderLabel.bounds.height + 4,
        width: SCREEN_WIDTH - 28, height: 15)
