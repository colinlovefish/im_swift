...
- (void)setDefaultValue {
    self.scale = 0.9;
    self.titlePoint = CGPointMake(0, 5);
    self.borderStyle = UITextBorderStyleNone;
    self.backgroundColor = [UIColor clearColor];
    self.lineSelectedColor = [UIColor blackColor];
    self.defaultTextAttributes = @{NSFontAttributeName:[UIFont systemFontOfSize:16],NSForegroundColorAttributeName:[UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:0.6]};
//    [self setAutocorrectionType:UITextAutocapitalizationTypeNone];
    [self setAutocorrectionType:UITextAutocorrectionTypeNo];
    
    self.isSelectedRegion = NO;
}
- (void)addSubView {
//    self.backgroundColor = UIColor.yellowColor;
    self.lineView.frame = CGRectMake(0, self.frame.size.height - 20, self.frame.size.width, 1);
...
- (void)setupSubViewFrame {
    self.lineView.frame = CGRectMake(0, self.frame.size.height - 20, self.frame.size.width, 1);
    self.animationLineView.frame = CGRectMake(0, self.frame.size.height - 20, 0, 1);
    self.placeholderCacheLabel.frame = CGRectMake(0, 0, self.bounds.size.width, 30);

    [self.errorLabel sizeToFit];
    self.errorLabel.frame = CGRectMake(self.errorLabelPoint.x, self.frame.size.height - self.errorLabel.frame.size.height - self.errorLabelPoint.y, self.frame.size.width,self.errorLabel.frame.size.height);
}
...
- (void)errorInfomation:(NSString *)text {
    ...
    self.lineView.backgroundColor = [UIColor colorWithRed:232 / 255.0 green:56 / 255.0 blue:47 / 255.0 alpha:1.0];
...
- (void)textFieldSelected:(BOOL) isSelected {
    if (isSelected) {
        self.isSelectedRegion = YES;
        [self bm_textFieldDidBeginEditing:nil];
    }else {
        self.isSelectedRegion = NO;
        [self bm_textFieldDidEndEditing:nil];
    }
}
...
- (void)bm_textFieldDidEndEditing:(NSNotification *)notification {
    ...
    if (!self.text || [self.text isEqualToString:@""]) {
        [self removePlaceholderAnimation];
//        if (_placeholderCacheLabel.hidden == true) {
//            
//        }
    }
    // 有验证并且没有通过
...
//        _placeholderCacheLabel.backgroundColor = [UIColor yellowColor];
    }
...
- (UILabel *)errorLabel {
    if (!_errorLabel) {
        ...
        _errorLabel.textColor = [UIColor colorWithRed:232 / 255.0 green:56 / 255.0 blue:47 / 255.0 alpha:1.0];
        _errorLabel.font = [UIFont systemFontOfSize:16];
        _errorLabel.hidden = YES;
        _errorLabel.text = @"input error";
